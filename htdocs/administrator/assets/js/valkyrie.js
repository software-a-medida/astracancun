/*!
 * Valkyrie v1.0.0 (https://codemonkey.com.mx/valkyrie-framework)
 * Copyright 2021 CodeMonkey Authors
...
!function ($) {
    ...
    Valkyrie.prototype.dropdowns = function () {
        $(document).on('click', '.dropmenu > button', function (event) {
            let button = $(this)

            event.stopPropagation()

            let dropmenu = button.parent()
    ...
    Valkyrie.prototype.modal = function () {
        $(document).on('click', '[data-button-modal]', function () {
            let button = $(this)
            let modal = button.data('button-modal')
            let send_data = button.data('send')
