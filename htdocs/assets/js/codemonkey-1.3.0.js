    ...
    ajax.send = function (data = new FormData()) {
        ...
        $.ajax({
            ...
            error: function (request, error) {
                ...
                if (typeof userSettings.httpError === 'function') {
                    userSettings.httpError(request, error)
                }
            },
            success: function (response, status, xhr) {
                ...
                if (response.status == 'error') {
                    if (typeof userSettings.onError === 'function') {
                        userSettings.onError(response, status, xhr)
                    }
                    ...
                    if (typeof userSettings.onLabelsError === 'function') {
                        userSettings.onLabelsError(response, status, xhr)
                    }
                } else if (response.status == 'fatal_error') {
                    if (typeof userSettings.onFatalError === 'function') {
                        userSettings.onFatalError(response, status, xhr)
                    }
                } else if (response.status == 'OK') {
                    if (typeof userSettings.success === 'function') {
                        userSettings.success(response, status, xhr)
                    }
